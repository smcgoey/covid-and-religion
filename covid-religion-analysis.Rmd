...
So it turns out that when we focus on heavily African American communities, we actually lose the slight negative correlation we had between number of congregations and COVID outbreaks nationwide.

There are a few outliers that I want to go back and look at, though. Let's redraw the nationwide plot of adherence vs. COVID:
...
The closest thing I can find to a thread tying these five counties together is that three of them -- Trousdale and Bledsoe counties in Tennessee and Lincoln County, Arkansas -- are home to prisons.